...
func CmdSetLogLevel(queryRoute string, cdc *codec.Codec) *cobra.Command {
	return &cobra.Command{
		Use:   "set-loglevel",
		Args:  cobra.ExactArgs(1),
		Short: "Set the omexchaind log level",
		Long: strings.TrimSpace(`
$ omexchaincli debug set-loglevel "main:info,state:info"

$ omexchaincli debug set-loglevel "upgrade:error"
`),
		RunE: func(cmd *cobra.Command, args []string) error {
			cliCtx := context.NewCLIContext().WithCodec(cdc)
			if len(args) > 1 {
			...
			fmt.Println("Succeed to set the omexchaind log level.")
			return nil
		},
	}
}
