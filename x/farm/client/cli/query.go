...
Example:
$ %s query farm rewards pool-eth-xxb omexchain1hw4r48aww06ldrfeuq2v438ujnl6alsz0685a0
`,
				version.ClientName,
			),
		),
...
func GetCmdQueryWhitelist(storeName string, cdc *codec.Codec) *cobra.Command {
	return &cobra.Command{
		Use:   "whitelist",
		Short: "query the whitelist of pools to farm omt",
		Long: strings.TrimSpace(
			fmt.Sprintf(`Query the current whitelist of pools which are approved to farm omt.
...
Example:
$ %s query farm account omexchain1hw4r48aww06ldrfeuq2v438ujnl6alsz0685a0
`,
				version.ClientName,
			),
		),
...
Example:
$ %s query farm lock-info pool-eth-xxb omexchain1hw4r48aww06ldrfeuq2v438ujnl6alsz0685a0 
`,
				version.ClientName,
			),
		),
