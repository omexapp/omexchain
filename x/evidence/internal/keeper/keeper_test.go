...
import (
	...
	"github.com/omexapp/omexchain/x/evidence/exported"
	"github.com/omexapp/omexchain/x/evidence/internal/keeper"
	"github.com/omexapp/omexchain/x/evidence/internal/types"
...
type KeeperTestSuite struct {
	...
	app     *app.OMExChainApp
}

func MakeOMEXApp() *app.OMExChainApp {
	genesisState := app.NewDefaultGenesisState()
	db := dbm.NewMemDB()
	omexapp := app.NewOMExChainApp(log.NewTMLogger(log.NewSyncWriter(os.Stdout)), db, nil, true, map[int64]bool{}, 0)

	stateBytes, err := codec.MarshalJSONIndent(omexapp.Codec(), genesisState)
	if err != nil {
		panic(err)
	}
	omexapp.InitChain(
