...
import (
	...
	dexUtils "github.com/omexapp/omexchain/x/dex/client/utils"
...
$ omexchaincli tx dex list --base-asset mytoken --quote-asset omt --from mykey
`),
...
$ omexchaincli tx dex deposit mytoken_omt 1000omt --from mykey

The 'product' is a trading pair in full name of the tokens: ${base-asset-symbol}_${quote-asset-symbol}, for example 'mytoken_omt'.
`),
...
$ omexchaincli tx dex withdraw mytoken_omt 1000omt --from mykey
...
$ omexchaincli tx dex register-operator --website http://xxx/operator.json --handling-fee-address addr --from mykey
`),
		RunE: func(cmd *cobra.Command, _ []string) error {
			cliCtx := context.NewCLIContext().WithCodec(cdc)
...
$ omexchaincli tx dex edit-operator --website http://xxx/operator.json --handling-fee-address addr --from mykey
`),
		RunE: func(cmd *cobra.Command, _ []string) error {
			cliCtx := context.NewCLIContext().WithCodec(cdc)
