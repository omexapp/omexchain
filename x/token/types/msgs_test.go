...
func TestNewTokenMultiSend(t *testing.T) {
	...
	// correct message
	coinStr := `[{"to":"omexchain1dfpljpe0g0206jch32fx95lyagq3z5ws850m6f","amount":"1` + common.NativeToken + `"}]`
	transfers, err := StrToTransfers(coinStr)
	require.Nil(t, err)

	// coins not positive
	toAddr0, err := sdk.AccAddressFromBech32("omexchain1dfpljpe0g0206jch32fx95lyagq3z5ws850m6f")
	require.Nil(t, err)
	...
	testCase := []struct {
		...
		{NewMsgMultiSend(fromAddr, transfers0), ErrInvalidCoins("0.000000000000000000omt")},
