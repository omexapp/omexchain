...
func TestOMExChainAppExport(t *testing.T) {
	db := dbm.NewMemDB()
	app := NewOMExChainApp(log.NewTMLogger(log.NewSyncWriter(os.Stdout)), db, nil, true, map[int64]bool{}, 0)
	...
	app2 := NewOMExChainApp(log.NewTMLogger(log.NewSyncWriter(os.Stdout)), db, nil, true, map[int64]bool{}, 0)
